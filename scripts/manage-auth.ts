...
function main() {
  ...
  switch (command) {
    case "add": {
      ...
      console.log(
        `Successfully added/updated user: "${username}" in docker/.htpasswd`,
      );
      break;
    }
    ...
    case "remove": {
      ...
      if (!users[username]) {
        console.error(
          `Error: User "${username}" not found in docker/.htpasswd`,
        );
        process.exit(1);
      }
      ...
      console.log(
        `Successfully removed user: "${username}" from docker/.htpasswd`,
      );
      break;
    }
    ...
    case "init-defaults": {
      ...
      for (const username of defaultUsers) {
        if (users[username]) {
          ...
          users[username] = hash;
          console.log(
            `Created user: "${username}" with temporary password: "${tempPassword}"`,
          );
        }
      }
      writeHtpasswd(users);
