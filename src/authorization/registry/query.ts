// src/authorization/registry/query.ts
 ...
 */
export function getPermissionsByVersion(
  version: string,
) {
  return ALL_PERMISSIONS.filter(
    permission =>
      permission.introducedIn === version,
  );
}
...
export function getDeprecatedPermissions() {
  return ALL_PERMISSIONS.filter(
    permission => permission.deprecatedIn,
  );
}
...
export function getRegistryStats() {
  return {
    ...
    systemPermissions:
      getSystemPermissions().length,

    deprecatedPermissions:
      getDeprecatedPermissions().length,

    hiddenPermissions:
      getHiddenPermissions().length,
  };
}