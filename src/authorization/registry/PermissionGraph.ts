...
export class PermissionGraph {
  ...
  constructor(permissions: PermissionDefinition[] = ALL_PERMISSIONS) {
   ...
   * Build graph.
   */
  private build(permissions: PermissionDefinition[]): void {
    for (const permission of permissions) {
      this.permissionMap.set(permission.key, permission);

      this.adjacencyList.set(permission.key, new Set());

      this.reverseAdjacencyList.set(permission.key, new Set());
    }
    ...
    for (const permission of permissions) {
      ...
      const edges = [...(deps.allOf ?? []), ...(deps.anyOf ?? [])];

      for (const dependency of edges) {
        this.adjacencyList.get(permission.key)!.add(dependency);

        this.reverseAdjacencyList.get(dependency)?.add(permission.key);
      }
    }
  }
  ...
  getDependents(key: string): string[] {
    return [...(this.reverseAdjacencyList.get(key) ?? [])];
  }
  ...
  getPermissionKeys(): string[] {
    return [...this.permissionMap.keys()];
  }
   ...
   * DFS traversal.
   */
  traverse(permissionKey: string, callback: (permission: PermissionDefinition) => void): void {
   ...
   */
  getRecursiveDependencies(permissionKey: string): string[] {
    const dependencies = new Set<string>();

    this.traverse(permissionKey, (permission) => {
  ...
  hasCycle(): boolean {
    ...
    const dfs = (permissionKey: string): boolean => {
      ...
      for (const dependency of this.getDependencies(permissionKey)) {
        if (dfs(dependency)) {
          return true;
        }
      }
