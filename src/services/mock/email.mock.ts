...
export async function sendVerificationEmail(opts: {
  to: string; name: string; userId: string; token: string;
}): Promise<void> {
  const link = `http://localhost:3001/verify-email?token=${opts.token}`;
  dummySend({ to: opts.to, subject: 'Verify your NexusBid email address', template: 'verification' });
  logger.info(
...
export async function sendAdminVerificationEmail(opts: {
  ...
  dummySend({ to: opts.to, subject: 'Verify your NexusBid Admin email address', template: 'admin-verification' });
  logger.info(
...
export async function sendSubscriptionReceiptEmail(opts: {
  ...
  planName: string; amountCents: number; expiresAt: Date;
}): Promise<void> {
  dummySend({
    to: opts.to,
...
export async function sendSubscriptionCancelledEmail(opts: {
  to: string; name: string; userId: string; planName: string; endsAt: Date;
}): Promise<void> {
  dummySend({ to: opts.to, subject: 'NexusBid: Subscription cancelled', template: 'subscription-cancelled' });
}
...
export async function sendEmailChangeVerificationEmail(opts: {
  ...
  dummySend({ to: opts.to, subject: 'Verify your new NexusBid email address', template: 'email-change-verification' });
  logger.info(
...
export async function sendEmailChangeAlertEmail(opts: {
  to: string; name: string; userId: string; newEmail: string;
}): Promise<void> {
  dummySend({ to: opts.to, subject: 'NexusBid Account Alert: Email change requested', template: 'email-change-alert' });
}

export async function sendLoginNotificationEmail(opts: {
  to: string; name: string; userId: string; ipAddress: string | null; userAgent: string | null; time: Date;
}): Promise<void> {
  dummySend({ to: opts.to, subject: 'Security Alert: New login detected on your NexusBid account', template: 'login-notification' });
}
...
export async function sendAdminRegistrationNotification(opts: {
  ...
  dummySend({ to: opts.to, subject: 'New Admin Registration Received', template: 'admin-registration-notification' });
  logger.info(
...
export async function sendAdminBootstrapNotification(opts: {
  ...
  dummySend({ to: opts.to, subject: 'Action Required: Bootstrap First Super Admin', template: 'admin-bootstrap-notification' });
  logger.info(
