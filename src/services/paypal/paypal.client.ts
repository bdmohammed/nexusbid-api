...
export async function paypalRequest<T>(
  ...
  if (env.NODE_ENV === "local") {
    logger.info(
      { path },
      "💳 [DUMMY TEST] PayPal request intercepted (local env)",
    );
    ...
    if (path.startsWith("/v1/billing/subscriptions")) {
      const returnUrl =
        bodyObj.application_context?.return_url || "http://localhost:3001";
      ...
      resVal = {
        ...
        links: [
          {
            href: `${returnUrl}?subscription=${subId}`,
            rel: "approve",
            method: "GET",
          },
        ],
      ...
      path.includes("/v2/checkout/orders/") &&
      path.endsWith("/capture")
    ) {
    ...
    logger.info(
      { path, durationMs, mock: true },
      "PayPal API request completed",
    );
    return resVal as T;
  }
