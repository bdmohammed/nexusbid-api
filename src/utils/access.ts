  ...
  if (validSubs.length > 0) {
    ...
    if (tender && tender.activeVersion) {
      ...
      for (const sub of validSubs) {
        ...
        if (
          plan.planType === "state" &&
          sub.targetStateId === version.stateId
        ) {
          return true;
        }
        ...
        if (
          ...
          version.state.country.toLowerCase() ===
            sub.targetCountry.toLowerCase()
        ) {
          return true;
        }
        ...
        if (
          plan.planType === "category" &&
          sub.targetCategoryId === version.categoryId
        ) {
          return true;
        }
