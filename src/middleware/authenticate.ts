...
export const authenticate = async (
  ...
  if (user.tokenVersion !== payload.tokenVersion) {
    return next(
      new AppError("Session has been revoked", 401, "SESSION_REVOKED"),
    );
  }
  ...
  if (
    ...
    !bypassForcedReset.some((route) => path.startsWith(route))
  ) {
    return next(
      new AppError(
  ...
  if (
    daysSinceChange > 90 &&
    !bypassExpiration.some((route) => path.startsWith(route))
  ) {
    return next(
      new AppError(
