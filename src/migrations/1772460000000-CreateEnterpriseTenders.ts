...
export class CreateEnterpriseTenders1772460000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE "tenders" ADD COLUMN "lifecycle_status" VARCHAR(50) DEFAULT 'ACTIVE';`);
    await queryRunner.query(`ALTER TABLE "tenders" ADD COLUMN "publication_status" VARCHAR(50) DEFAULT 'PUBLISHED';`);
    ...
    for (const ot of oldTenders) {
      ...
      const versionStatus = ot.status === 'under_review' ? 'UNDER_REVIEW' : (ot.status === 'draft' ? 'DRAFT' : 'APPROVED');
      const pubStatus = ot.status === 'active' ? 'PUBLISHED' : (ot.status === 'draft' ? 'SCHEDULED' : ot.status.toUpperCase());
      ...
      `, [
        ot.title, ot.description, ot.city || '', ot.agency || '', ot.eligibility || '',
        ot.contact_info || '', ot.posted_date ? new Date(ot.posted_date).toISOString() : new Date().toISOString(),
      ...
      if (ot.document_s3_key) {
        ...
        `, [ot.document_s3_key, ot.document_s3_bucket || 'nexusbid-tenders', ot.document_original_name || 'document.pdf']);
      }
    }
    ...
    await queryRunner.query(`ALTER TABLE "tender_versions" ADD CONSTRAINT "fk_tender_versions_tenders" FOREIGN KEY ("tender_id") REFERENCES "tenders"("id") ON DELETE CASCADE;`);
    await queryRunner.query(`ALTER TABLE "tenders" ADD CONSTRAINT "fk_tenders_active_version" FOREIGN KEY ("active_version_id") REFERENCES "tender_versions"("id") ON DELETE SET NULL;`);
