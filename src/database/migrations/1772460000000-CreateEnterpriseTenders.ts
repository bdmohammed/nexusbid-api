...
export class CreateEnterpriseTenders1772460000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE SEQUENCE IF NOT EXISTS tender_ref_seq START WITH 100001;`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "tenders" ADD COLUMN "active_version_id" UUID;`,
    );
    await queryRunner.query(
      `ALTER TABLE "tenders" ADD COLUMN "reference_no" VARCHAR(100) UNIQUE;`,
    );
    await queryRunner.query(
    ...
    for (const ot of oldTenders) {
      ...
      const refNo =
        ot.ref_number ||
        `TDR-2026-${Math.floor(Math.random() * 900000 + 100000)}`;
      ...
      `,
        [
          ...
          ot.contact_info || "",
          ot.posted_date
            ? new Date(ot.posted_date).toISOString()
            : new Date().toISOString(),
          ot.deadline
            ? new Date(ot.deadline).toISOString()
            : new Date().toISOString(),
          ot.priceCents || 0,
        ],
      );
    ...
    await queryRunner.query(
      `ALTER TABLE "tenders" DROP COLUMN "contact_info";`,
    );
    await queryRunner.query(
      `ALTER TABLE "tenders" DROP COLUMN "submission_type";`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "tenders" DROP COLUMN "document_s3_key";`,
    );
    await queryRunner.query(
      `ALTER TABLE "tenders" DROP COLUMN "document_s3_bucket";`,
    );
    await queryRunner.query(
      `ALTER TABLE "tenders" DROP COLUMN "document_original_name";`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "tenders" DROP COLUMN "rejection_note";`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "tenders" RENAME COLUMN "lifecycle_status" TO "status";`,
    );
