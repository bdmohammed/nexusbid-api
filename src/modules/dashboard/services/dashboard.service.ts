...
export async function getDashboardConfig(
  ...
  const authorizedWidgets = WIDGET_REGISTRY.filter((w) => {
    ...
    return (
      userPermissions.includes(w.requiredPermission) ||
...
export async function getTenderData() {
  ...
  const closingTodayCount = await tenderRepo
    ...
    .where("activeVersion.closingDate BETWEEN :todayStart AND :todayEnd", {
      todayStart,
      todayEnd,
    })
    .getCount();
...
export async function getUsersData() {
  ...
  const pendingApprovals = await userRepo.count({
