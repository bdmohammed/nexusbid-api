 ...
 */
export const redirectToProvider = asyncHandler(
...
export const handleCallback = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const profile = await oauthService.verifyCallbackAndGetUser(
        provider,
        code,
      );
      ...
      res.redirect(
        `${env.FRONTEND_CUSTOMER_URL}/login?error=${encodeURIComponent(errMsg)}`,
      );
    }
  },
);
