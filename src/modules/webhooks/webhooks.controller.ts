...
export const handlePayPalWebhook = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    logger.info({ eventId, eventType, paypalSubscriptionId, paypalOrderId }, 'PayPal webhook received');
    ...
    if (alreadyProcessed) {
      logger.info({ eventId, eventType, paypalSubscriptionId, paypalOrderId }, 'Duplicate PayPal webhook — ignoring');
      return res.status(200).json({ received: true });
    }
