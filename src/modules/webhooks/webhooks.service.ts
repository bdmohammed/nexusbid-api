...
async function handleSubscriptionCancelledOrExpired(
  ...
  if (!sub) {
    logger.warn(
      { paypalSubscriptionId },
      "Subscription not found for cancellation event",
    );
    return;
  }
  ...
  logger.info(
    { paypalSubscriptionId },
    "Subscription marked cancelled/expired",
  );
}
...
async function handleSaleCompleted(
  ...
  if (!sub) {
    logger.warn(
      { billingAgreementId },
      "Subscription not found for PAYMENT.SALE.COMPLETED",
    );
    return;
  }
...
async function handleSaleDenied(
  ...
  logger.warn(
    { billingAgreementId },
    "Subscription payment denied — marked expired",
  );
}
...
async function handleCaptureCompleted(
  ...
  if (sub) {
    ...
    logger.info(
      { orderId, subId: sub.id },
      "Activated subscription from capture",
    );
  }
}
